...
class PacketAccessor {
    ...
    static {
        ...
        try {
            ...
            if (CAULDRON_SERVER) {
                ...
                System.out.println("🔍 Fields in PacketPlayOutScoreboardTeam:");
                ...
                if (MINOR_VERSION >= 21 && PATCH_VERSION >= 5) {
                    playerConnection = typeNMSPlayer.getField("f");
                } else if (MINOR_VERSION >= 21 && PATCH_VERSION >= 2) {
                    playerConnection = typeNMSPlayer.getField("f");
            ...
            if (currentVersion != null) {
                if (!isParamsVersion()) {
                    ...
                    try {
                        String paramFieldName = currentVersion.getParams();
                        Field paramField = packetClass.getDeclaredField(paramFieldName);
    ...
    private static Field getParamNMS(String path) throws Exception {
        try {
            ...
            field.setAccessible(true);
            System.out.println("✅ Found param field: " + path + " (type: " + field.getType().getName() + ")");
            return field;
        } catch (NoSuchFieldException e) {
            System.out.println("❌ Failed to find param field: " + path);
            throw e;
        }
    }
